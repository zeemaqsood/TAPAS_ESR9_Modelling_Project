globals [element-scheme color-scheme color-scheme2 effective-receptors-link cluster-count size-of-cluster  total-size mean-size
  time-step-counter  element-state-counts-list   coeff-of-variance-list   mean-zero-list   steady-state-checker ]
...
ligands-own [element-state product-id product-turn product-speed receptor-bound ligand-bound dimer-bound is-dimer? reversible? molecule l1 l2 cluster-id]
;dimers-own [family product-id product-turn site1-bounded?  site2-bounded?]
receptors-own [element-state product-id product-turn product-speed phosphorylated? ligand-bound protein-bound receptor-bound molecule cluster-id ]
syks-own [element-state product-id product-turn product-speed kinectically-active? inter-complex? site1-bound  site2-bound molecule cluster-id ]
...
to setup-globals
  ; each element's colors are mapped in the same order for both lists: element-scheme and color-scheme
  set element-scheme ["L0" "L1" "L0L0" "L1L0" "L1L1" "R0" "R1" "R2" "R3" "R4" "R5" "S0" "S1" "S2" "S3" "S4" "S5"]
  set color-scheme [ white blue gray white white gray blue violet blue violet violet white blue violet red white white ]
  set color-scheme2 [ gray yellow gray yellow yellow yellow yellow yellow yellow yellow violet gray white violet red white white ]
  set effective-receptors-link limit-rr-links
  if limit-rr-links = "unlimited"
  [ set effective-receptors-link R ]
  set-patch-size 1
  resize-world -400 400 -400 400
  ; for steady state
  set element-state-counts-list []
  set coeff-of-variance-list []
  set mean-zero-list []
  set steady-state-checker []
  set time-step-counter 0
  ; creating a list of lists for count of families
  foreach element-scheme
  [
    set element-state-counts-list  lput [] element-state-counts-list
    set coeff-of-variance-list lput [] coeff-of-variance-list
    set mean-zero-list lput [] mean-zero-list
    set steady-state-checker lput false steady-state-checker
  ]
end
...
; setting values for monomeric ligand
to setup-monomers
  create-ligands L
  [
    set shape "monomer"
    ...
    set l2 nobody
    set cluster-id nobody
    setxy random-xcor random-ycor
   ]
end

; to get the element state of monomeric ligand
...
to setup-receptors
  ...
  [
    ...
    if random-float 100 < initial-R1-presence
    [
      set phosphorylated? true
    ]
...
; to have fifty patches together and each set of fifty patches will be
; randomly located.
to  setup-slow-region
  ...
  [
    ...
    let yellow-space-num cnt-patches / 400              ; number of different regions to be formed which are of size 50
...
to go
  if ((ticks = stopwatch) or is-steady-state?)
  [ stop ]
  move
...
to  phosphorylation-process
  ...
  [
    ...
    let rate-list  item 1 theme-rates              ; to get if the process is applicable under the applicable the theme
    let transform-list? will-transform? rate-list
    ; if theme-1 is enabled and it could be transformed (true in rate-list)
    ; then we can phosphorylate the receptor, similar logic from themes 5 and 10.
...
to dimerization-of-r
  ...
  [
    ...
    [
      ...
      ; add the receptor bound to self of others
...
to attach-syk
  ...
  [
    ...
    ifelse site1-bound = nobody
    [ set site1-bound [self] of the-molecule ]
    [ if site2-bound = nobody [set site2-bound [self] of the-molecule ] ]
    set element-state get-syk-element-state
    set color get-color
...
;  ask ligands with [is-dimer? and not reversible? ]
;  [
;    let near-by-molecules other ligands-here with [is-dimer? and product-id != [product-id] of myself]
;    if any? near-by-molecules and random-float 100 < rate-for-theme-6
;    [
;      let near-by-molecule one-of near-by-molecules
;      set color get-color
        ...
        [
          ...
          [
            ...
            [ set receptor-bound remove me receptor-bound
              if empty? receptor-bound [ __set-line-thickness -4 ]  ]
            [ show "not member"]
          ]
...
; reverse reaction logic implementation for all reactions
to  unphosphorylation-process
  ...
  let theme-list  item 0 theme-rates
  let rate-list  item 1 theme-rates
  set rate-list (update-rate-list rate-list)   ; to reduce the speed of clusters based on count of receptors
  let transform-list? will-transform? rate-list
  if member? "theme-1" theme-list
  [
    let pos position "theme-1" theme-list
...
to-report update-rate-list [ xrate-list ]
  let new-rate-list []
  foreach xrate-list
  [ rate ->
    let speed-reducer (1 - base-reduction)
    ifelse cluster-id != nobody
    [
      let count-receptors count receptors with [cluster-id = [cluster-id] of myself]
      ifelse count-receptors < 4
      [ set speed-reducer 1 ]
      [ ;
        let diff (count-receptors - 4 )
        let times 0
        while [ times < diff ]
        [
          set times (times + 1)
          set speed-reducer (1 - (base-reduction + (times * step-reduction)))
        ]
      ]
    ]
    [
    ...
    set new-rate-list lput (rate * speed-reducer) new-rate-list
  ]
  report new-rate-list
end
...
to write-into-file
  ...
  set line (word behaviorspace-run-number "," ticks "," tot-phos-receptors ","  cluster-count "," mean-size ","  receptors-in-cluster "," receptors-out-cluster "," double-syks-in-cluster "," double-syks-out-cluster)
  file-print line
    file-close ]
end
...
to set-cluster-number
  set cluster-count 0
  set size-of-cluster []
  let unique-products []
  ask receptors
  [
    let size-cluster count receptors with [product-id = [product-id] of myself]
    ifelse size-cluster >= 4
    [
      if not member? product-id unique-products
      [
        set cluster-count (cluster-count + 1)
        set size-of-cluster lput size-cluster size-of-cluster
        set unique-products lput product-id unique-products
    ...
    [
      ask turtles with [product-id = [product-id] of myself and cluster-id != nobody ]
      [ set cluster-id nobody ]
    ]
  ]
  set total-size 0
...
to-report receptors-in-cluster
  report count receptors with [ phosphorylated? and cluster-id != nobody  ]
end
...
to-report double-syks-in-cluster
  report count syks with [ site1-bound != nobody and site2-bound != nobody and  cluster-id != nobody]
end
...
to-report is-steady-state?
  ifelse check-steady-state? [
  set time-step-counter (time-step-counter + 1)
  foreach element-scheme
  [ element ->
    let pos position element element-scheme
    let element-state-count-values item pos element-state-counts-list
    let coeff-of-variance-values item pos coeff-of-variance-list
    let current-count count turtles with [element-state = element ]
    set element-state-count-values  fput current-count element-state-count-values
    set element-state-counts-list  replace-item pos element-state-counts-list element-state-count-values
    if (time-step-counter mod step-size) = 0
    [
      if (length element-state-count-values) >= consecutive-states
      [
        let recent-counts sublist element-state-count-values 0 consecutive-states
        let element-state-mean mean recent-counts
        let element-state-stddev standard-deviation recent-counts
        ifelse element-state-mean = 0
        [ let mean-zero item pos mean-zero-list
          set mean-zero fput 0 mean-zero
          if length mean-zero >= consecutive-zero
          [ set steady-state-checker replace-item pos steady-state-checker true ]
          set mean-zero-list replace-item pos mean-zero-list mean-zero
        ]
        [
          set mean-zero-list replace-item pos mean-zero-list []
          let coeff-of-var element-state-stddev /  element-state-mean
          set coeff-of-variance-values fput coeff-of-var coeff-of-variance-values
          set coeff-of-variance-list replace-item pos coeff-of-variance-list coeff-of-variance-values
          if length coeff-of-variance-values >= consecutive-states
          [
            let no-diff 0
            foreach sublist coeff-of-variance-values 0 consecutive-states
            [ cov ->
              if cov <= epsilon
              [
                set no-diff no-diff + 1
              ]
            ]
            if no-diff >= consecutive-states
            [set steady-state-checker replace-item pos steady-state-checker true]
          ]
        ]
      ]
  ...
  if member? false steady-state-checker[ report false]
...
"" ""
PENS
"Cluster Number" 1.0 0 -5298144 true "" "plot cluster-count"
"Mean Size" 1.0 0 -4079321 true "" "plot mean-size"
"phos-R % in Cluster" 1.0 0 -13840069 true "" "plot ( receptors-in-cluster / R) * 100"
"phos-R % Out Cluster" 1.0 0 -7500403 true "" "plot receptors-out-cluster / R * 100"
"Double-bound S % in Cluster " 1.0 0 -2674135 true "" "if S > 0 [ plot double-syks-in-cluster / S * 100 ]"
"Double-bound S % Out Cluster" 1.0 0 -955883 true "" "if S > 0 [ plot double-syks-out-cluster / S * 100 ]"
"Total Phos Receptors" 1.0 0 -6759204 true "" "plot count receptors with [ phosphorylated? ] / R * 100"
"dimerised R" 1.0 0 -2064490 true "" "plot count receptors with [ not empty? receptor-bound] / R * 100"
...
677
Dimerised R %
Count receptors with [ not empty? receptor-bound] / R * 100
1
1
16
...
750
phos-R %
count receptors with [ phosphorylated? ] / R * 100
17
1
16
...
true
0
Polygon -7500403 true true 15 210 150 0 285 210 105 210 15 210

receptor-with-border
true
0
Polygon -7500403 true true 15 210 150 0 285 210 120 210 120 210
Line -1184463 false 150 0 15 210
Line -1184463 false 15 210 285 210
Line -1184463 false 150 0 285 210
...
Rectangle -16777216 false false 90 135 210 150
...
<experiments>
  <experiment name="experiment" repetitions="50" runMetricsEveryStep="true">
    ...
    <metric>Count receptors with [ not empty? receptor-bound] / R * 100</metric>
    ...
    <enumeratedValueSet variable="theme-1">
      <value value="false"/>
    </enumeratedValueSet>
    ...
    <enumeratedValueSet variable="theme-2">
      <value value="true"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="S">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="theme-3">
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="L">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="step-reduction">
      <value value="0.01"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="check-steady-state?">
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="rate-for-theme-5">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="coefficient-of-diffusion">
      <value value="0.056"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="rate-for-theme-6">
      <value value="1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="consecutive-states">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="rate-for-theme-7">
      <value value="86"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="slowing-region?">
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="LL">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="rate-for-theme-8">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="reverse-rate-for-theme-5">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="epsilon">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="reverse-rate-for-theme-6">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="rate-for-theme-9">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="rate-for-theme-1">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="reverse-rate-for-theme-7">
      <value value="50"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="rate-for-theme-2">
      <value value="10"/>
      <value value="1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="reverse-rate-for-theme-8">
      <value value="1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="limit-rr-links">
      <value value="&quot;unlimited&quot;"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="rate-for-theme-3">
      <value value="5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="theme-9">
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="reverse-rate-for-theme-9">
      <value value="1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="consecutive-zero">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="base-reduction">
      <value value="0.05"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="rate-for-theme-4">
      <value value="100"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="reverse-rate-for-theme-1">
      <value value="1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="reverse-rate-for-theme-2">
      <value value="1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="stopwatch">
      <value value="3400"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="reverse-rate-for-theme-3">
      <value value="1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="reverse-rate-for-theme-10">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="rate-for-theme-10">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="theme-4">
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="reverse-rate-for-theme-4">
      <value value="1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="theme-5">
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="slow-area">
      <value value="25"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="theme-10">
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="theme-6">
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="theme-7">
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="theme-8">
      <value value="false"/>
    </enumeratedValueSet>
  </experiment>
</experiments>
